...
export default function WhyChooseUs({ data, translations }: WhyChooseUsProps) {
  if (!data && !translations.defaultTitle) return null

  const title = translations.defaultTitle || data?.title || ""
  const buttonText = translations.defaultButtonText || data?.buttonText || ""
  const buttonUrl = data?.buttonUrl || "#"
  const imageUrl = data?.imageUrl || "/default-image.webp"
  ...
  return (
    <section id="kapec-mes" className="relative py-16 sm:py-20 px-6 sm:px-12 bg-gradient-to-br from-white via-[#F9FAFB] to-white overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto flex flex-col lg:flex-row items-center gap-12 lg:gap-20">
        {/* Saturs - ar nelieliem uzlabojumiem */}
        <div className="relative bg-white/95 backdrop-blur-xl rounded-3xl shadow-2xl shadow-[#77dDB4]/15 p-8 lg:p-10 lg:max-w-[600px] w-full border border-[#77dDB4]/20 group hover:shadow-3xl hover:shadow-[#77dDB4]/25 transition-all duration-500 hover:scale-105">
     
          <h2 className="text-3xl sm:text-4xl lg:text-4xl font-bold text-[#00332D] mb-8 leading-tight">
            {title}
          </h2>
          
          <div className="space-y-5 text-[#00332D] text-lg sm:text-xl mb-10">
            {points.map((item, i) => (
              <div key={i} className="group/item flex items-start gap-4 p-4 rounded-xl hover:bg-[#77dDB4]/5 transition-all duration-300">
                <div className="relative flex-shrink-0 mt-1">
                  <div className="w-7 h-7 bg-gradient-to-r from-[#77dDB4] to-[#5BC9A8] rounded-full flex items-center justify-center group-hover/item:scale-110 transition-transform duration-300 shadow-lg">
                    <CheckCheck className="w-4 h-4 text-white" />
                  </div>
                </div>
                <span className="font-medium leading-relaxed group-hover/item:text-[#00443B] transition-colors duration-300">
                  {translations[`point${i + 1}`] || item}
                </span>
              </div>
            ))}
          </div>
          ...
          {buttonUrl && buttonText && (
            <div className="relative">
              ...
              <a href={buttonUrl}
                className="group/btn relative inline-flex items-center gap-3 bg-gradient-to-r from-[#00332D] to-[#00443B] text-white px-8 py-4 rounded-xl font-semibold text-lg shadow-xl shadow-[#00332D]/30 hover:shadow-2xl hover:shadow-[#00332D]/40 transition-all duration-300 transform hover:scale-105 border-2 border-transparent hover:border-[#77dDB4] overflow-hidden"
              >
                ...
                <div className="absolute inset-0 bg-gradient-to-r from-[#77dDB4]/30 to-[#77dDB4]/20 transform scale-x-0 group-hover/btn:scale-x-100 transition-transform duration-300 origin-left"></div>
              </a>
            </div>
          )}
        </div>
        
        {/* Attēla sadaļa - ar nelieliem uzlabojumiem */}
        <div className="relative w-full flex-grow group/image">
          <div className="relative aspect-[3/2] sm:aspect-[4/3] rounded-2xl overflow-hidden shadow-2xl shadow-[#77dDB4]/20 border-4 border-white group-hover/image:shadow-3xl group-hover/image:shadow-[#77dDB4]/30 transition-all duration-500 group-hover/image:scale-105">
            <Image
              src={imageUrl}
              alt={translations.defaultImageAlt}
              fill
